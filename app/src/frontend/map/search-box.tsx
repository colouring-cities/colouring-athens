...
interface SearchBoxProps {
  onLocate: (lat: number, lng: number, zoom: number, extent: any) => void;
}
...
class SearchBox extends Component<SearchBoxProps, SearchBoxState> {
  ...
  render() {
    ...
    let type_a_postcode = "Type a postcode...";
    let search = 'Search';
      ...
      <ul className="search-box-results">
        {this.state.results.map((result) => {
          ...
          const zoom = result.attributes.zoom;
            const bounds = [
                [result.geometry.coordinates[0][0][1],result.geometry.coordinates[0][0][0]],
                [result.geometry.coordinates[0][2][1],result.geometry.coordinates[0][2][0]],
            ];
          const href = `?bounds=${bounds}`;
          ...
          return (
            <li key={result.attributes.label}>
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  this.props.onLocate(lat, lng, zoom, bounds);
                }}
                href={href}
    ...
    if (i18next.language === "gr") {
        return (
            <div className="search-box" onKeyDown={this.handleKeyPress}>
              <div className="search-box-pane">
                <div
                  className={`collapse-btn ${this.state.smallScreen ? "active" : ""}`}
                  onClick={this.state.smallScreen ? this.expandSearch : null}
                >
                  <SearchIcon />
                </div>
                <form
                  onSubmit={this.search}
                  className="form-inline d-flex flex-nowrap"
                >
                  <input
                    ...
                    name="q"
                    value={this.state.q}
                    placeholder={type_a_postcode}
                    aria-label={type_a_postcode}
