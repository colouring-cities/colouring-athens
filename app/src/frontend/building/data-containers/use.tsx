...
const UseView: React.FunctionComponent<CategoryViewProps> = (props) => {
  ...
  return (
    <Fragment>
      ...
      {props.mode != "view" && <InfoBox msg={t("land_use_info")}></InfoBox>}

      <DataEntryUse
            title={t("building_use")}
            // tooltip={dataFields.current_landuse_order.tooltip}
            slug="current_use"
            value={props.building.current_landuse_ground_floor === props.building.current_landuse_floor ? props.building.current_landuse_ground_floor : 14}
            mode={props.mode}
            // disabled={true}
            // copy={props.copy}
            // onChange={props.onChange}
