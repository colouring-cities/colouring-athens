...
import DataEntry, {
  BaseDataEntryProps,
} from "../../data-components/data-entry";
...
export const DynamicsBuildingPane: React.FC<{ className?: string }> = ({
  children,
  className,
}) => (
  <div className={`dynamics-building-pane ${className ?? ""}`}>{children}</div>
);
...
const DynamicsDataRow: React.FC<DynamicsDataRowProps> = ({
  ...
  const maxLifespan = lifespan(
    value.year_demolished?.max,
    value.year_constructed?.min
  );
  const minLifespan = lifespan(
    value.year_demolished?.min,
    value.year_constructed?.max
  );

  let year_constructed_title =
    dataFields.demolished_buildings.items.year_constructed.title_en;
  let year_demolished_title =
    dataFields.demolished_buildings.items.year_constructed.title_en;
      ...
      <FieldRow>
        <div>
          <NumberRangeDataEntry
            ...
            titleMin={`${year_constructed_title}: earliest estimate`}
            titleMax={`${year_constructed_title}: latest estimate`}
            required={required}
          />
        </div>
        <div>
          <NumberRangeDataEntry
            ...
            placeholderMin="Earliest"
            placeholderMax="Latest"
            titleMin={`${year_demolished_title}: earliest estimate`}
            titleMax={`${year_demolished_title}: latest estimate`}
            required={required}
          />
        </div>
...
export const DynamicsDataEntry: React.FC<DynamicsDataEntryProps> = (props) => {
  ...
  const edit = useCallback(
    (id: number, val: DemolishedBuilding) => {
      ...
      <div>
        {isEditing ? (
          <>
            ...
            <label>
              Please supply sources for any edits of existing records
            </label>
          </>
        ) : (
          <DataTitleCopyable
        ...
        <ul className="data-entry-list">
          {values.length === 0 && (
            <div className="input-group">
              <input
                className="form-control no-entries"
                type="text"
                value="No records so far"
                disabled={true}
              />
            </div>
          )}
            ...
            <li key={id}>
              <DynamicsBuildingPane>
                ...
                {!isDisabled && (
                  <button
                    type="button"
                    className="btn btn-outline-dark delete-record-button"
                    title="Delete Record"
                    onClick={() => remove(id)}
                    data-index={id}
                  >
        ...
        {!isDisabled && (
          <div className="new-record-section">
            ...
            <DynamicsBuildingPane className="new-record">
              ...
              />
              <label>
                Please save all your edits before navigating away from the
                currently selected building - these will be erased otherwise.
              </label>
              <button
                type="button"
